    ...
    if head1 and head2:
        for index, x in enumerate(header1):
            if header1[index] == header2[index]:
                header1[index] = '{}<span style="background-color:#008000">{}'.format(header1[index][:4],header1[index][4:])
                header2[index] = '{}<span style="background-color:#008000">{}'.format(header2[index][:4],header2[index][4:])
    if header1:
