...
def _setup_argparser():
    ...
    for item in INSTALL_CANDIDATES:
        install_options.add_argument(f'-{item[0].upper()}', f'--{item}', action='store_true', default=False, help=f'install {item}')
...
def install_statistic_cronjob():
    ...
    output, return_code = execute_shell_command_get_return_code(f'crontab {crontab_file_path}')
    if return_code != 0:
