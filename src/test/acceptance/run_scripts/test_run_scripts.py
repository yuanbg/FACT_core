...
def test_start_script_help_and_version(script, expected_str):
    output, return_code = execute_shell_command_get_return_code(f'{os.path.join(get_src_dir(), script)} -h', timeout=5)
    assert return_code == 0
    assert f'usage: {script}' in output

    output, return_code = execute_shell_command_get_return_code(f'{os.path.join(get_src_dir(), script)} -V', timeout=5)
    assert expected_str in output, f'Wrong output {output}'
    assert return_code == 0
...
def test_fact_complete_start():
    output, return_code = execute_shell_command_get_return_code(f'{os.path.join(get_src_dir(), "start_fact.py")} -d -t')
