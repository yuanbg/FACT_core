...
class TestScheduleInitialAnalysis(AnalysisSchedulerTest):
    ...
    def test_schedule_firmware_init_no_analysis_selected(self):
        ...
        test_fo = FileObject(binary=b'test')
        self.sched.add_task(test_fo)
        test_fo = self.sched.process_queue.get(timeout=5)
        assert len(test_fo.scheduled_analysis) == len(MANDATORY_PLUGINS), 'Mandatory Plugins not selected'
        for item in MANDATORY_PLUGINS:
            assert item in test_fo.scheduled_analysis

    def test_whole_run_analysis_selected(self):
        test_fo = FileObject(file_path=os.path.join(get_test_data_dir(), 'get_files_test/testfile1'))
        test_fo.scheduled_analysis = ['dummy_plugin_for_testing_only']
        self.sched.add_task(test_fo)
        for _ in range(3):  # 3 plugins have to run
            test_fo = self.tmp_queue.get(timeout=10)
        assert len(test_fo.processed_analysis) == 3, 'analysis not done'
        assert test_fo.processed_analysis['dummy_plugin_for_testing_only']['1'] == 'first result', 'result not correct'
        assert test_fo.processed_analysis['dummy_plugin_for_testing_only']['summary'] == ['first result', 'second result']
        assert 'file_hashes' in test_fo.processed_analysis.keys(), 'Mandatory plug-in not executed'
        assert 'file_type' in test_fo.processed_analysis.keys(), 'Mandatory plug-in not executed'

    def test_expected_plugins_are_found(self):
        result = self.sched.get_plugin_dict()
        assert 'file_hashes' in result.keys(), 'file hashes plugin not found'
        assert 'file_type' in result.keys(), 'file type plugin not found'
        assert 'dummy_plug_in_for_testing_only' not in result.keys(), 'dummy plug-in not removed'
    ...
    def test_process_next_analysis_unknown_plugin(self):
        test_fw = FileObject(file_path=os.path.join(get_test_data_dir(), 'get_files_test/testfile1'))
    ...
    def test_skip_analysis_because_whitelist(self):
        ...
        test_fo = FileObject(file_path=os.path.join(get_test_data_dir(), 'get_files_test/testfile1'))
        test_fo.scheduled_analysis = ['file_hashes']
        test_fo.processed_analysis['file_type'] = {'mime': 'text/plain'}
        self.sched._start_or_skip_analysis('dummy_plugin_for_testing_only', test_fo)
        test_fo = self.tmp_queue.get(timeout=10)
        assert 'dummy_plugin_for_testing_only' in test_fo.processed_analysis
        assert 'skipped' in test_fo.processed_analysis['dummy_plugin_for_testing_only']
...
class TestAnalysisSchedulerBlacklist:
    ...
    class PluginMock:  # pylint: disable=invalid-name
    ...
    def test_get_blacklist_and_whitelist_from_plugin__missing_in_plugin(self):
        ...
        _, whitelist = self.sched._get_blacklist_and_whitelist_from_plugin('test_plugin')
        assert whitelist == []
...
class TestAnalysisSkipping:
    ...
    class BackendMock:  # pylint: disable=no-self-use
