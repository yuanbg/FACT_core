...
class TestStorageDbInterfaceAdmin:
    ...
    def setup_method(self):
        ...
        self.test_firmware, self.test_root_fo = create_test_firmware(bin_path='container/test_copy.zip')
        self.uid = self.test_root_fo.get_uid()
        self.test_firmware.virtual_file_path = {self.uid: ['|{}|'.format(self.test_root_fo.get_uid())]}
        ...
        self.test_root_fo.files_included = [self.child_fo.get_uid()]
    ...
    def test_remove_object_field(self):
        ...
        result = self.db_backend_interface.file_objects.find_one(self.child_uid, {'virtual_file_path': 1})
        assert self.uid in result['virtual_file_path']

        self.admin_interface.remove_object_field(self.child_uid, 'virtual_file_path.{}'.format(self.uid))
        result = self.db_backend_interface.file_objects.find_one(self.child_uid, {'virtual_file_path': 1})
        assert self.uid not in result['virtual_file_path']
    ...
    def test_delete_swapped_analysis_entries(self):
        self.test_root_fo.processed_analysis = {'test_plugin': {'result': 10000000000, 'misc': 'delete_swap_test'}}
        self.db_backend_interface.add_file_object(self.test_root_fo)
        self.admin_interface.client.drop_database(self.config.get('data_storage', 'sanitize_database'))
        self.admin_interface.sanitize_analysis(self.test_root_fo.processed_analysis, self.uid)
        assert 'test_plugin_result_{}'.format(self.test_root_fo.get_uid()) in self.admin_interface.sanitize_fs.list()
        self.admin_interface._delete_swapped_analysis_entries(self.admin_interface.file_objects.find_one(self.uid))
        assert 'test_plugin_result_{}'.format(self.test_root_fo.get_uid()) not in self.admin_interface.sanitize_fs.list()
    ...
    def test_delete_file_object(self):
        ...
        assert self.db_backend_interface.file_objects.find_one(self.child_fo.get_uid()) is None, 'file not deleted from db'
        delete_tasks = self._get_delete_tasks()
        assert self.child_fo.get_uid() in delete_tasks, 'file not found in delete tasks'

    def test_delete_firmware(self):
        self.db_backend_interface.add_firmware(self.test_firmware)
        self.db_backend_interface.add_file_object(self.test_root_fo)
        self.db_backend_interface.add_file_object(self.child_fo)
        assert self.db_backend_interface.firmware_metadata.find_one({'_id': self.test_firmware.firmware_id}) is not None
        assert self.db_backend_interface.file_objects.find_one(self.uid) is not None
        assert self.db_backend_interface.file_objects.find_one(self.child_uid) is not None
        assert os.path.isfile(self.test_root_fo.file_path)
        assert os.path.isfile(self.child_fo.file_path)

        removed_vps, deleted_files = self.admin_interface.delete_firmware(self.test_firmware.firmware_id)
        # self.assertIsNone(self.db_backend_interface.firmwares.find_one(self.uid), 'firmware not deleted from db')
        assert self.db_backend_interface.firmware_metadata.find_one({'_id': self.test_firmware.firmware_id}) is None, 'firmware metadata not deleted from db'
        assert self.db_backend_interface.file_objects.find_one(self.uid) is None, 'firmware fo not deleted from db'
        ...
        delete_tasks = self._get_delete_tasks()
        assert self.test_root_fo.get_uid() in delete_tasks, 'fw root fo delete task not found'
        assert self.child_fo.get_uid() in delete_tasks, 'child delete task not found'
